        ...
        for lig in self.ligList:
            ...
            if self.bDSSB==True:
                for case,state in zip(['holo','apo'],self.states): 
                    ...
                    gmx.grompp(f=mdp, c=waterPdb, p=outTop, o=tpr, maxwarn=10, other_flags=' -po {0}'.format(mdout), gmxexec=self.gmxexec, verbose=self.bVerbose) 
...
#            tpr = '{0}/ti{1}.tpr'.format(simpath,frNum)        
        ...
        gmx.grompp(f=mdp, c=inStr, p=top, o=tpr, maxwarn=10, other_flags=' -po {0}'.format(mdout), gmxexec=self.gmxexec, verbose=self.bVerbose)
